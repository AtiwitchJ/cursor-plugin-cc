...
test("codex marketplace entry points at the plugin source", () => {
  ...
  assert.equal(parsed.name, "agents-plugin-cc-cursor");
  const entry = parsed.plugins.find((plugin) => plugin.name === "cursor");
  assert.ok(entry, "Missing cursor entry");
  assert.equal(entry.source.source, "local");
  assert.equal(entry.source.path, "./plugins/cursor");
