...
test("marketplace.json exists and is valid JSON", () => {
  ...
  assert.equal(parsed.plugins[0].name, "cursor");
});
...
test("every command file exists", () => {
  ...
  const cmdDir = path.join(ROOT, "plugins", "cursor", "commands");
...
test("rescue subagent exists", () => {
  const agentFile = path.join(ROOT, "plugins", "cursor", "agents", "cursor-rescue.md");
