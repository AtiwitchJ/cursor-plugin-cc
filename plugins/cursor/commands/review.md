...
Run a Cursor agent review through the shared plugin runtime.
...
- Your only job is to run the review and return Cursor agent's output verbatim to the user.
...
- After launching, tell the user: "Cursor agent review started in the background. Check `/cursor:status` for progress."