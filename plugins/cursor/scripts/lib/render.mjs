...
export function renderTaskResult({ text, failureMessage, reasoningSummary }, { title, jobId = null, write = false, agentName = "Cursor" } = {}) {
  const lines = [
    `# ${title ?? "Kilo task"}`,
...
export function renderReviewResult(text, { reviewLabel = "Review", targetLabel = "working tree", sessionId = null, agentName = "Cursor" } = {}) {
  const lines = [
    `# ${reviewLabel}`,
    `- Target: ${targetLabel}`,
...
export function renderStoredJobResult(job, storedJob, { agentName = "Cursor" } = {}) {
...
export function renderTransferResult({ threadId, resumeCommand, agentName = "Cursor" }) {
  return [
    `# ${agentName} transfer`,
